...
async function ensureUserRecord(
  ...
  try {
    ...
    const userData: UserRecord = {
      id: authUser.id,
      telegram_id: telegramUser ? telegramUser.id.toString() : 'test_user',
      username: telegramUser ? telegramUser.username || `tg_${telegramUser.id}` : 'test_user',
      full_name: telegramUser 
        ? `${telegramUser.first_name} ${telegramUser.last_name || ''}`.trim()
        : 'Test User'
    };
...
export async function ensureUser(): Promise<User> {
  ...
  try {
    ...
    if (existingUser) {
      await ensureUserRecord(supabase, existingUser);
      return existingUser;
    }
    ...
    await ensureUserRecord(supabase, signInData.user);
    return signInData.user;
